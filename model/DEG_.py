...
class DEG(nn.Module):
    def __init__(self, field_num, emb_dim, bit_layers=3, mlp_layer=256, att_size=16):
        ...
        # self.use_bn = True
        # if self.use_bn:
        #     self.bn1 = nn.BatchNorm1d(embed_input_dim)
        #     self.bn2 = nn.BatchNorm1d(embed_input_dim)
    ...
    def forward(self, x_emb):
        ...
        # if self.use_bn:
        #     dnn_out, cn_out = self.bn1(dnn_out.flatten(start_dim=1)), self.bn2(cn_out.flatten(start_dim=1))
...
class SelfAttention(nn.Module):
    ...
    #     for m in [self.trans_Q,self.trans_K,self.trans_V, self.projection]:
