...
class EDCN(BasicCTR):
    def __init__(self, 
        ...
        # MLP = linear + activation + dropout # batch_norm=False
        self.dense_layers = nn.ModuleList([MLP_Block(input_dim=self.hidden_dim, # MLP = linear + activation + dropout
                                                     output_dim=None, 
        ...
        self.regulation_modules = nn.ModuleList([RegulationModule(len(field_dims), 
                                                                  ...
                                                                  use_bn=batch_norm) \
                                                 for _ in range(num_cross_layers)])
        self.fc = nn.Linear(self.hidden_dim * 3, 1) # 输出的时候三个输出向量 然后合成一个 
    ...
    def forward(self, input_ids):
        ...
        for i in range(len(self.cross_layers)):
            ...
            bridge_i = self.bridge_modules[i](cross_i, deep_i) # 合并操作 
            if i + 1 < len(self.cross_layers):
                cross_i, deep_i = self.regulation_modules[i + 1](bridge_i) # 学习权重 门控 
...
class MultiLayerPerceptron_(nn.Module):
    def __init__(self, input_dim, embed_dims, dropout=0.5, output_layer=False):
        super().__init__()
        layers = list()
        for embed_dim in embed_dims: # MLP = linear + BatchNorm1d + activation + dropout
        ...
        # 使用 *，
        self.mlp = torch.nn.Sequential(*layers) # * used to unpack list
        self._init_weight_()
